...
class _BrowserPageState extends State<BrowserPage> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _displayDomain = _extractDomain(widget.initialUrl ?? "https://www.google.com");

    _bannerAnimController = AnimationController(
      vsync: this, duration: const Duration(milliseconds: 500),
    );
    _bannerAnim = CurvedAnimation(parent: _bannerAnimController, curve: Curves.easeOut);
  ...
  void _showSnackBar(String msg, {bool isError = false, VoidCallback? action, String? actionLabel}) {
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        action: action != null
            ? SnackBarAction(label: actionLabel ?? 'View', textColor: GPColors.accent, onPressed: action)
            : null,
      ),
    );
  }
        ...
        child: Column(
          children: [
            ...
            if (_encryptionInitialized)
              FadeTransition(opacity: _bannerAnim, child: _buildSecurityBanner()),
          ],
        ),
      ),
    );
      ...
      child: Row(
        children: [
          ...
          _buildNavBtn(
            ...
            enabled: true,
            onTap: () => _isLoading ? _controller?.stopLoading() : _controller?.reload(),
          ),
                ...
                child: Row(
                  children: [
                    Icon(
                      ...
                      size: 12,
                      color: _isEditingUrl ? GPColors.primaryLight : Colors.white70,
                    ),
                    const SizedBox(width: 6),
                    Expanded(
                      child: _isEditingUrl
                          ? TextField(
                              ...
                              style: const TextStyle(
                                fontSize: 13, color: GPColors.textPrimary,
                              ),
                    ...
                    if (_isEditingUrl && _urlController.text.isNotEmpty)
                      GestureDetector(
                        ...
                        child: const Icon(Icons.cancel_rounded, size: 15, color: GPColors.textMuted),
                      ),
  ...
  Widget _buildNavBtn({required IconData icon, required bool enabled, required VoidCallback onTap}) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Icon(icon, size: 15, color: enabled ? Colors.white : Colors.white30),
      ),
    );
  }
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    const Text('Quick Access', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: GPColors.textPrimary)),
                  ],
                ),
                ...
                Container(
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [
                      BoxShadow(color: GPColors.primary.withOpacity(0.04), blurRadius: 6, offset: const Offset(0, 2)),
                    ],
                  ),
                      ...
                      Expanded(
                        child: TextField(
                          ...
                          onSubmitted: (val) {
                            ...
                            _controller?.loadUrl(urlRequest: URLRequest(url: WebUri(_smartUrl(val))));
                          },
                        ),
                      ),
                      GestureDetector(
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(title, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: GPColors.textPrimary)),
                  Text(subtitle, style: const TextStyle(fontSize: 11, color: GPColors.textMuted)),
                ],
              ),
            ),
            const Icon(Icons.arrow_forward_ios_rounded, size: 12, color: GPColors.textMuted),
          ],
        ),
      ),
    );
  ...
  Widget _buildStatusBar() {
    ...
    return Container(
      ...
      color: isDone ? GPColors.surfaceTint : isError ? const Color(0xFFFFF3F3) : GPColors.surfaceTint,
      child: Row(
        children: [
          if (isDone)
            ...
            const SizedBox(
              width: 13, height: 13,
              child: CircularProgressIndicator(strokeWidth: 1.5, color: GPColors.primaryMid),
            ),
          const SizedBox(width: 8),
          Expanded(
            child: Text(
  ...
  Widget _buildSecurityBanner() {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Icon(Icons.verified_user_rounded, size: 12, color: GPColors.primaryLight),
          const SizedBox(width: 6),
          Text(
            'AES-256 encrypted downloads · GenuPort Secure Browser',
            style: TextStyle(fontSize: 11, color: GPColors.primaryMid, letterSpacing: 0.1),
          ),
        ],
      ),
    );
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                ...
                style: TextStyle(fontSize: 13.5, color: GPColors.textMuted, height: 1.5),
              ),
              const SizedBox(height: 20),
              SizedBox(
                width: double.infinity,
  ...
  Future<void> _handleDownload(DownloadStartRequest request, InAppWebViewController controller) async {
    try {
      if (!_encryptionInitialized) throw Exception('Encryption not initialized');
      ...
      _showSnackBar(
        ...
        action: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const DownloadsPage())),
        actionLabel: 'View',
      );
  ...
  Future<File> _downloadBlobUrl(DownloadStartRequest request, InAppWebViewController controller) async {
    if (!await _requestStoragePermission()) throw Exception('Storage permission denied');
    ...
    if (fileName.toLowerCase().endsWith('.pdf')) finalBytes = await _checkPdf(finalBytes);
  ...
  Future<File> _saveEncryptedFile(Uint8List encryptedBytes, String fileName) async {
    ...
    if (Platform.isAndroid) {
      ...
      directory = Directory('${d.path}/GenuPortDownloads');
    }
    ...
    while (File(filePath).existsSync()) {
      ...
      if (parts.length > 1) {
        finalFileName = "${parts.sublist(0, parts.length - 1).join('.')}_$counter.${parts.last}.enc";
      } else {
